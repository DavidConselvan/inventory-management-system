...
export function ProductsPage() {
  ...
  const columns: Column<Product>[] = [
    ...
    { key: 'sku', header: 'SKU', sortValue: (p) => p.sku, render: (p) => <Badge variant="default">{p.sku}</Badge> },
    { key: 'unit', header: 'Unit', sortValue: (p) => p.unit, render: (p) => p.unit.toLowerCase() },
    {
      key: 'actions',
      header: '',
      align: 'right',
      ...
      <Card withBorder radius="md" p="md">
        <QueryBoundary isLoading={products.isLoading} isError={products.isError} error={products.error}>
          <DataTable
            ...
            columns={columns}
            getRowKey={(p) => p.id}
            searchText={(p) => `${p.name} ${p.sku} ${p.description}`}
